...
const char *WINDOWTITLE = { "OpenGL / GLUT Sample -- Chun Han Chen" };
...
//const float Sun_ORBIT_TIME_DAYS		= 36.3;
const float Mercury_ORBIT_TIME_DAYS = 100/0.241;
const float Venus_ORBIT_TIME_DAYS	= 100/0.615;
const float Moon_ORBIT_TIME_DAYS	= 100/0.073;
const float Earth_ORBIT_TIME_DAYS	= 100.;
const float Mars_ORBIT_TIME_DAYS	= 100/1.881;
const float Jupiter_ORBIT_TIME_DAYS	= 100/11.86;
const float Saturn_ORBIT_TIME_DAYS	= 100/29.46;
const float Uranus_ORBIT_TIME_DAYS	= 100/84.01;
const float Neptune_ORBIT_TIME_DAYS = 100/164.8;

const float Sun_SPIN_TIME_DAYS		= 100/27.;
const float Mercury_SPIN_TIME_DAYS	= 100/58.6;
const float Venus_SPIN_TIME_DAYS	= 100/243.;
const float Moon_SPIN_TIME_DAYS		= 100/27;
const float Earth_SPIN_TIME_DAYS	= 100/0.99;
const float Mars_SPIN_TIME_DAYS		= 100/1.03;
const float Jupiter_SPIN_TIME_DAYS	= 100/0.41;
const float Saturn_SPIN_TIME_DAYS	= 100/0.45;
const float Uranus_SPIN_TIME_DAYS	= 100/0.72;
const float Neptune_SPIN_TIME_DAYS	= 100/0.67;

void PlanetRing(float r1, float r2, int n)
{
	float angle_orig = 0.;
...
Display()
{
	...
	{
		//scorpio point1
		SetPointLight(GL_LIGHT1, 0., 0., 0., 0., 0., 1.);
		glPushMatrix();
		glDisable(GL_LIGHTING);
		glColor3f(0., 0., 1.);
		glTranslatef(15., 1.2, 6.9);
		glScalef(1., 1., 1.);
	...
	glRotatef(Sun_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	glCallList(SunList);
	glPopMatrix();
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Mercury_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(6., 0., 0.);
	glRotatef(Mercury_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Venus_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(8.25, 0., 0.);
	glRotatef(Venus_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Earth_ORBIT_TIME_DAYS* Time, 0., 1., 0.);
	glTranslatef(10.5, 0., 0.);
	glRotatef(Earth_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	
	glCallList(EarthList);
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Moon_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(-0.8, 0., 0.);
	glRotatef(Moon_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Mars_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(12., 0., 0.);
	glRotatef(Mars_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	glCallList(MarsList);
	glPopMatrix();
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Jupiter_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(22., 0., 0.);
	glRotatef(Jupiter_SPIN_TIME_DAYS* Time, 0., 1., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Saturn_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(40., 0., 0.);
	glRotatef(Saturn_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Uranus_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(50., 0., 0.);
	glRotatef(Uranus_SPIN_TIME_DAYS * Time, 0., 1., 0.);
	...
	glRotatef(Uranus_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(50., 0., 0.);
	glCallList(UranusringList);
	glPopMatrix();

	//Neptune
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glRotatef(Neptune_ORBIT_TIME_DAYS * Time, 0., 1., 0.);
	glTranslatef(60., 0., 0.);
	glRotatef(Neptune_SPIN_TIME_DAYS * Time, 0., 1., 0.);
...
InitMenus( )
{
	...
	int viewmenu = glutCreateMenu(DoWhichViewMenu);
...
InitGraphics( )
{
	...
	//Sun texture
	int width, height;
	unsigned char* Texture = BmpToTexture("8k_sun.bmp", &width, &height);
	...
	//Mercury texture
	int width1, height1;
	unsigned char* Texture1 = BmpToTexture("8k_mercury.bmp", &width1, &height1);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Venus texture
	int width2, height2;
	unsigned char* Texture2 = BmpToTexture("8k_venus_surface.bmp", &width2, &height2);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Moon texture
	int width3, height3;
	unsigned char* Texture3 = BmpToTexture("8k_moon.bmp", &width3, &height3);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Earth texture
	int width4, height4;
	unsigned char* Texture4 = BmpToTexture("8k_earth_daymap.bmp", &width4, &height4);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Mars texture
	int width5, height5;
	unsigned char* Texture5 = BmpToTexture("8k_mars.bmp", &width5, &height5);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Jupiter texture
	int width6, height6;
	unsigned char* Texture6 = BmpToTexture("8k_jupiter.bmp", &width6, &height6);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Saturn texture
	int width7, height7;
	unsigned char* Texture7 = BmpToTexture("8k_saturn.bmp", &width7, &height7);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Uranus texture
	int width8, height8;
	unsigned char* Texture8 = BmpToTexture("2k_uranus.bmp", &width8, &height8);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	//Neptune texture
	int width9, height9;
	unsigned char* Texture9 = BmpToTexture("2k_neptune.bmp", &width9, &height9);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	unsigned char* Texture10 = BmpToTexture("Stars.bmp", &width10, &height10);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, width10, height10, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture10);

	//Uranus ring texture
	int width11, height11;
	unsigned char* Texture11 = BmpToTexture("uranusring.bmp", &width11, &height11);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, width11, height11, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture11);

	//Saturn ring texture
	int width12, height12;
	unsigned char* Texture12 = BmpToTexture("8k_saturn_ring_alpha.bmp", &width12, &height12);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexImage2D(GL_TEXTURE_2D, 0, 3, width12, height12, 0, GL_RGB, GL_UNSIGNED_BYTE, Texture12);
...
InitLists( )
{
	...
	glBindTexture(GL_TEXTURE_2D, Uranustexture);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	OsuSphere(1.039, 50., 50.);
	glDisable(GL_TEXTURE_2D);
	glEndList();

	//Uranus ring
	UranusringList = glGenLists(1);
	glNewList(UranusringList, GL_COMPILE);
	glEnable(GL_TEXTURE_2D);
	glBindTexture(GL_TEXTURE_2D, Uranusringtexture);
	glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
	glRotatef(90., 0., 0., 1.);
